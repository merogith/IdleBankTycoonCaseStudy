...
├── index.html                             ← the case-study site (single source for the PDF)
...
├── docs/
│   └── case-study.pdf                     ← build output of index.html — run scripts/build-pdf.sh
...
> The PDF is generated from `index.html`, not maintained by hand. After editing the site, run `scripts/build-pdf.sh` (needs Chrome/Chromium) to regenerate it so the two never drift apart.
